...
public class ODMeta extends ODNode {
     ...
     * @return the requested metadata, or <code>null</code> if none is found and
     *         <code>create</code> is <code>false</code>.
     */
    public final ODUserDefinedMeta getUserMeta(String name, boolean create) {
        try {
            final XPath path = this.getParent().getXPath("./" + ODUserDefinedMeta.getPathTo(name));
            final Element userElem = (Element) path.selectSingleNode(this.getElement());
            if (userElem != null)
    ...
    public final List<String> getUserMetaNames() throws JDOMException {
        final XPath path = this.getParent().getXPath("./" + ODUserDefinedMeta.getNamesPath());
        final List<Attribute> attrs = path.selectNodes(this.getElement());
        final List<String> res = new ArrayList<String>(attrs.size());
