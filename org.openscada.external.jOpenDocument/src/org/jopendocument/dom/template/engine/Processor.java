 ...
 * <dd>the following expression will be converted to a String with toString() and then encoded
 * using {@link OOXML#encodeWS(String)}.</dd>
...
public class Processor<E> {
        ...
        if (tagName.equals("text-input")) {
            ...
            if (tag != null) {
                try {
                    ...
                    throw new TemplateException("pb at " + JDOMUtils.output(element), e);
                }
            } else {
    ...
    private void replaceField(Element field) throws TemplateException {
        ...
        if (value != null) {
            if (encode)
                value = OOXML.get(this.ns.getVersion()).encodeWS(value.toString());
            else if (isOOXML) {
                try {
