...
public class Row<D extends ODDocument> extends TableCalcNode<RowStyle, D> {
    ...
    Row(Table<D> parent, Element tableRowElem, int index) {
        super(parent.getODDocument(), tableRowElem);
        this.parent = parent;
        this.index = index;
    ...
    public final MutableCell<D> getMutableCellAt(final int col) {
        ...
        if (!(c instanceof MutableCell)) {
            ...
            this.cells.set(col, new MutableCell<D>(this, element, col));
        }
    ...
    void columnCountChanged() {
        ...
        if (diff < 0) {
            ...
            final Element e = Cell.createEmpty(this.getSheet().getODDocument().getNS(), diff);
