...
public class TestTable {
    ...
    public static void main(String[] args) {
        try {
            ...
            final ODSingleXMLDocument single = new ODPackage(templateFile).toSingle();

            // Growing
            {
                final Element table = single.getDescendantByName("table:table", "GrowingTable");
            ...
            // Fixed
            {
                final Element table = single.getDescendantByName("table:table", "FixedTable");
                final Table<ODSingleXMLDocument> t = new Table<ODSingleXMLDocument>(single, table);
                // this will bring the column count to 5
                // the new columns will be created with the same style as column 1
            ...
            // Shrinking
            {
                final Element table = single.getDescendantByName("table:table", "ShrinkingTable");
                final Table<ODSingleXMLDocument> t = new Table<ODSingleXMLDocument>(single, table);
            ...
            // Fixed
            {
                final Element table = single.getDescendantByName("table:table", "FixedTable2");
                final Table<ODSingleXMLDocument> t = new Table<ODSingleXMLDocument>(single, table);
